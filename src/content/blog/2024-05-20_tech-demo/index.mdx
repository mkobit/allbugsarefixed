...
Import icons from **Lucide React**, or pass icon names as strings (resolved dynamically). Use the `IconBlock` component to create featured icon grids with consistent styling.

<div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-6">
  <IconBlock client:visible icon={Zap} label="Component reference" variant="warning" />
  <IconBlock client:visible icon="activity" label="String icon name" variant="info" />
...
> [!info] Obsidian Callout Test
> This callout was automatically generated from standard markdown syntax.
...
bun install super-cool-package
```
...
```typescript title="src/pages/index.astro" showLineNumbers path="/src/pages/index.astro" copy="false" runnable="true" mode="dark"
...
Use the `<Map>` component to render config-driven maps via **Pigeon Maps**. Map configurations should be colocated in a typed `*-data.ts` module (e.g. `map-data.ts`) to preserve TypeScript type checking.
...
Authors can opt-in to upgrade any standard Markdown table into an interactive, sortable `<DataTable>` by placing `{/* datatable */}` on its own line immediately before the table:
...
Standard Markdown tables without `{/* datatable */}` remain plain static HTML tables.

For custom cell rendering or complex schemas, the hand-authored `<DataTable>` component is also supported:
