                    ...
                    <th key={header.id} className={head()}>
                      {header.isPlaceholder ? null : (
                        ...
                        >
                          ...
                          }[header.column.getIsSorted() as string] ?? (header.column.getCanSort() ? <ChevronsUpDown className="h-4 w-4 text-gray-400" /> : null)}
                        </div>
                      )}
                    </th>
                  )
          ...
          <tbody className={body()}>
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((rowEl) => (
                <tr
                  key={rowEl.id}
                  data-state={rowEl.getIsSelected() && "selected"}
                  className={row()}
                >
