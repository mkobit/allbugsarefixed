...
2. For each issue, run `bd show <id>` and check for missing or weak metadata:
   - No type set: infer from title/description and set with `bd update <id> --type <task|feature|bug|epic|chore|decision>`
   - No priority set: infer urgency from description and set with `bd priority <id> <0-4>`
   - Title is vague (e.g. "fix", "update", "thing"): improve it with `bd update <id> --title "..."`
   - No description: add one with `bd update <id> --description "..."` or `bd note <id> "<summary of what this actually is>"`
