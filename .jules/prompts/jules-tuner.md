...
4. Review the full set of prompts and identify any maintenance concern that recurs but has no prompt:
   - Examples: orphaned bd issues, openspec changes with no linked bd issues, issues stuck in `in_progress` for too long, `blog-lifecycle` molecules stalled at one stage for a long time.
