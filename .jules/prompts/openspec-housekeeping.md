...
2. For each change, run `bunx openspec change show <name>` to check which artifacts exist (proposal/specs/design/tasks/retrospective/reflection — this repo's `beads-driven` schema, see `openspec/schemas/beads-driven/schema.yaml`, expects all six before archiving).
...
3. For each change still in progress:
   Run `bunx openspec validate <name> --no-interactive` to catch structural issues.
...
- Do not run `bunx openspec change validate` in create mode or otherwise propose anything new.
