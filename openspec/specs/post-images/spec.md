...
Define the requirements for post image handling in blog content, including single image Astro asset optimization via plain markdown, a `Gallery` grid layout component for multiple images, and an accessible interactive `Lightbox` modal for full-size image viewing.
