...
> **Note (`abf-zdv.4.18`):** scenarios below describe the file shape as originally implemented (`notebook.md` + `index.mdx`, two files). This was superseded by a single-file model — see `design.md` Decisions 5–7 for the current mechanism. Scenarios updated in place below to reflect the superseding design; historical rationale for the change lives in `design.md`, not here.
